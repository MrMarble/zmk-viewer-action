...
const modVersionRe = /github.com\/mrmarble\/zmk-viewer\s(v.+)/
...
const getRequestedLintVersion = (): Version => {
    ...
    if (requestedLintVersion == "" && fs.existsSync(goMod)) {
        ...
        if (match) {
            ...
            core.info(`Found zmk-viewer version '${requestedLintVersion}' in '${goMod}' file`)
        }
    }
    ...
    if (isLessVersion(parsedRequestedLintVersion, minVersion)) {
        throw new Error(
            `requested zmk-viewer version '${requestedLintVersion}' isn't supported: we support only ${stringifyVersion(
                minVersion
            )} and later versions`
        )
    }
...
const getConfig = async (): Promise<Config> => {
    const http = new httpm.HttpClient(`mrmarble/zmk-viewer-action`, [], {
        allowRetries: true,
        maxRetries: 5,
    })
    try {
        const url = `https://raw.githubusercontent.com/mrmarble/zmk-viewer/master/assets/github-action-config.json`
...
export async function findLintVersion(): Promise<VersionConfig> {
    core.info(`Finding needed zmk-viewer version...`)
    ...
    if (reqLintVersion?.major !== null && reqLintVersion?.minor != null && reqLintVersion?.patch !== null) {
        return new Promise((resolve) => {
            ...
            resolve({
                ...
                AssetURL: `https://github.com/mrmarble/zmk-viewer/releases/download/v${versionWithoutV}/zmk-viewer-${versionWithoutV}-linux-amd64.tar.gz`,
            })
        })
    }
    ...
    if (!versionConfig) {
        throw new Error(`requested zmk-viewer version '${stringifyVersion(reqLintVersion)}' doesn't exist`)
    }

    if (versionConfig.Error) {
        throw new Error(`failed to use requested zmk-viewer version '${stringifyVersion(reqLintVersion)}': ${versionConfig.Error}`)
    }

    core.info(
        `Requested zmk-viewer '${stringifyVersion(reqLintVersion)}', using '${versionConfig.TargetVersion}', calculation took ${
            Date.now() - startedAt
        }ms`
    )
